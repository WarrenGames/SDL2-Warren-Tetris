...
{
	std::size_t matWidth{0};
	std::size_t matHeight{0};
	...
	if( inputFile >> matWidth >> matHeight >> colorIndex )
	{
		...
		logs.error << "Error: in '" << filePath << "' file. Reading mat width , height and color index error.\n";
	}
}

void loadPieceMatrixData(AppLogFiles& logs, const std::string& fileName, std::ifstream& inputFile, MatrixTemp2D<SquareData>& matrix, LoadedPieces& pieces, std::size_t color)
{
	bool data{0};
	for(std::size_t sqr_y{0} ; sqr_y < matrix.height() ; ++sqr_y )
	{
		for( std::size_t sqr_x{0} ; sqr_x < matrix.width() ; ++sqr_x )
		{
			if( inputFile >> data )
			{
