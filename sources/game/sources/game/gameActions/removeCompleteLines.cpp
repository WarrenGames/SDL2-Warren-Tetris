...
void countCompleteLines(const MatrixTemp2D<SquareData>& matrix, ScanLinesInfos& scanLinesInf, size_t& completeLinesNum)
{
	...
	for( size_t height{scanLinesInf.topLine} ; height <= scanLinesInf.bottomLine ; ++height )
	{
...
void decalOverCompleteLine(MatrixTemp2D<SquareData>& matrix, ScanLinesInfos& scanLinesInf)
{
	assert( scanLinesInf.bottomLine < matrix.height() );
	assert( scanLinesInf.topLine < matrix.height() );
	for( size_t height{scanLinesInf.bottomLine} ; height >= scanLinesInf.topLine ; --height )
	{
		if( height >= 3 && height < matrix.height() )
		{
...
{
	for( size_t height{startBottom} ; height > 0 ; --height )
	{
