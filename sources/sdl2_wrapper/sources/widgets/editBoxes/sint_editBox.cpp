...
{
	if( events.getSpecificKey(BOX_EV_BACKSPACE) && false == stringOfSint.empty() )
	{
...
{
	if( events.getSpecificKey(BOX_EV_RETURN) && false == stringOfSint.empty() && !isOnlyMinusCharInSintString(stringOfSint) )
	{
		quitEdition = true;
		hasStringChanged = true;
...
void SintEditBox::appendNewFigureToSintString()
{
	...
	{
		if( events.getNumbersState(i) && stringOfSint.size() < CHARSIZEMAX )
		{
...
{
	if( stringOfSint.empty() && events.getSpecificKey(BOX_EV_MINUS) )
	{
		addFigure("-");
		events.setSpecificKeyToFalse(BOX_EV_MINUS);
	}
}
...
void SintEditBox::quitSintEdition(bool& quitEdition)
{
	...
	&& ( (!isUserMouseInBox(boxRect.getMainRect(), events.getMousePosition() ) && events.getMouseLeftButtonState() ) || events.getSpecificKey(BOX_EV_ESCAPE) ) )
		quitEdition = true;
}
