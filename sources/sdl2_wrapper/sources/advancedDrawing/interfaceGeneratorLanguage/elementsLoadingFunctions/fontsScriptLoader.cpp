...
{
	if( data.identifier == igl::font::ARIAL_STD_PATH )
	{
...
{
	if( data.identifier == igl::font::NEURO_FONT_STD_PATH )
	{
...
{
	if( data.identifier == igl::font::FONT_PATH )
	{
		if( !( lineStream >> data.fontPath ) )
		{
			logs.error << "Error: custom font path '" << igl::font::FONT_PATH << "' identifier was read but reading following font custom path failed.\n";
...
{
	if( data.identifier == igl::font::PROPORTIONAL_SIZE )
	{
...
{
	if( data.identifier == igl::font::STATIC_SIZE )
	{
		unsigned readValue{0};
		if( lineStream >> readValue )
		{
